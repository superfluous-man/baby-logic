...
\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0111"><strong>(\#cnj:prob0111) </strong></span>List the <bb>element</bb>s of the relation $\subseteq$ on the set $\mathscr{P}({\lbrace a, b, c\rbrace })$.</div>\EndKnitrBlock{conjecture}
...
\BeginKnitrBlock{proposition}<div class="proposition"><span class="proposition" id="prp:prpPorder"><strong>(\#prp:prpPorder) </strong></span>If $R$ is a partial order on $X$, then $R^- = R \setminus \text{Id}_X$ is a strict order. Moreover, if $R$ is linear, then $R^-$ is total.</div>\EndKnitrBlock{proposition}
...
![](https://i.loli.net/2020/09/03/Eg9Ium37BZiHy62.png)
...
![](https://i.loli.net/2020/09/04/5nBmbKUJP2aeDc9.png)
...
![](https://i.loli.net/2020/09/04/PpOsUwucAmM3qhv.png)
...
\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0121"><strong>(\#cnj:prob0121) </strong></span>Show that if $f \colon A \to B$ and $g \colon B \to C$ are both <bb>surjective</bb>, then $g\circ f\colon A \to C$ is <bb>surjective</bb>.</div>\EndKnitrBlock{conjecture}
...
\BeginKnitrBlock{example}<div class="example"><span class="example" id="exm:exmPartfunction02"><strong>(\#exm:exmPartfunction02) </strong></span>The partial function $f \colon \mathbb{R} \nrightarrow \mathbb{R}$ given by $f(x) = 1/x$ is undefined for $x = 0$, and defined everywhere else.</div>\EndKnitrBlock{example}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0123"><strong>(\#cnj:prob0123) </strong></span>Given $f\colon A \nrightarrow B$, define the partial function $g\colon B \nrightarrow A$ by: for any $y \in B$, if there is a unique $x \in A$ such that $f(x) = y$, then $g(y) = x$; otherwise $g(y) \uparrow$.  Show that if $f$ is injective, then $g(f(x)) = x$ for all $x \in \text{dom}(f)$, and $f(g(y)) = y$ for all $y \in \text{ran}(f)$.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{definition}\iffalse{-91-71-114-97-112-104-32-111-102-32-97-32-112-97-114-116-105-97-108-32-102-117-110-99-116-105-111-110-93-}\fi{}<div class="definition"><span class="definition" id="def:defGPartfunction"><strong>(\#def:defGPartfunction)  \iffalse (Graph of a partial function) \fi{} </strong></span>Let $f\colon A \nrightarrow B$ be a partial function. The <bb>graph</bb> of $f$ is the relation $R_f \subseteq A \times B$ defined by $$R_f = \lbrace ⟨x,y⟩: f(x) = y\rbrace.$$</div>\EndKnitrBlock{definition}

\BeginKnitrBlock{proposition}<div class="proposition"><span class="proposition" id="prp:prpGPartfunction"><strong>(\#prp:prpGPartfunction) </strong></span>Suppose $R \subseteq A \times B$ has the property that whenever $Rxy$ and $Rxy'$ then $y = y'$.  Then $R$ is the graph of the partial function $f\colon X \nrightarrow Y$ defined by: if there is a $y$ such that $Rxy$, then $f(x) = y$, otherwise $f(x) \uparrow$.  If $R$ is also <bb>serial</bb>, i.e., for each $x \in X$ there is a $y \in Y$ such that $Rxy$, then $f$ is total.</div>\EndKnitrBlock{proposition}

\BeginKnitrBlock{proof}<div class="proof">\iffalse{} <span class="proof"><em>Proof: </em></span>  \fi{}Suppose there is a $y$ such that $Rxy$.  If there were another $y' \neq y$ such that $Rxy'$, the condition on $R$ would be violated. Hence, if there is a $y$ such that $Rxy$, that $y$ is unique, and so $f$ is well-defined.  Obviously, $R_f = R$ and $f$ is total if $R$ is serial.</div>\EndKnitrBlock{proof}
...
\BeginKnitrBlock{definition}\iffalse{-91-69-110-117-109-101-114-97-116-105-111-110-44-32-105-110-102-111-114-109-97-108-108-121-93-}\fi{}<div class="definition"><span class="definition" id="def:defEnumeration"><strong>(\#def:defEnumeration)  \iffalse (Enumeration, informally) \fi{} </strong></span>Informally, an <bb>enumeration</bb> of a set $A$ is a list (possibly infinite) of <bb>element</bb>s of $A$ such that every <bb>element</bb> of $A$ appears on the list at some finite position. If $A$ has an enumeration, then $A$ is said to be <bb>enumerable</bb>.</div>\EndKnitrBlock{definition}
...
\BeginKnitrBlock{proposition}<div class="proposition"><span class="proposition" id="prp:prpenumeration"><strong>(\#prp:prpenumeration) </strong></span>If $A$ has an enumeration, it has an enumeration without repetitions.</div>\EndKnitrBlock{proposition}

\BeginKnitrBlock{proof}<div class="proof">\iffalse{} <span class="proof"><em>Proof: </em></span>  \fi{}Suppose $A$ has an enumeration $x_1$, $x_2, \cdots$, in which each $x_i$ is an <bb>element</bb> of $A$.  We can remove repetitions from an enumeration by removing repeated <bb>element</bb>s. For instance, we can turn the enumeration into a new one in which we list $x_i$ if it is an <bb>element</bb> of $A$ that is not among $x_1, \cdots, x_{i-1}$ or remove $x_i$ from the list if it already appears among $x_1, \cdots, x_{i-1}$.</div>\EndKnitrBlock{proof}
...
\BeginKnitrBlock{definition}\iffalse{-91-69-110-117-109-101-114-97-116-105-111-110-44-32-102-111-114-109-97-108-108-121-93-}\fi{}<div class="definition"><span class="definition" id="def:defEnumerationform"><strong>(\#def:defEnumerationform)  \iffalse (Enumeration, formally) \fi{} </strong></span>An <bb>enumeration</bb> of a set $A \neq \varnothing$ is any <bb>surjective</bb> function $f \colon \mathbb{Z}^{+} \to A$.</div>\EndKnitrBlock{definition}
...
\BeginKnitrBlock{definition}<div class="definition"><span class="definition" id="def:defenumerable"><strong>(\#def:defenumerable) </strong></span>A set $A$ is <bb>enumerable</bb> iff it is empty or has an enumeration.</div>\EndKnitrBlock{definition}

\BeginKnitrBlock{example}<div class="example"><span class="example" id="exm:exmEnumeration01"><strong>(\#exm:exmEnumeration01) </strong></span>A function enumerating the positive integers ($\mathbb{Z}^{+}$) is simply the identity function given by $f(n) = n$. A function enumerating the natural numbers $\mathbb{N}$ is the function $g(n) = n - 1$.</div>\EndKnitrBlock{example}

\BeginKnitrBlock{example}<div class="example"><span class="example" id="exm:exmEnumeration02"><strong>(\#exm:exmEnumeration02) </strong></span>The functions $f\colon \mathbb{Z}^{+} \to \mathbb{Z}^{+}$ and $g \colon \mathbb{Z}^{+} \to \mathbb{Z}^{+}$ given by
...
enumerate the even positive integers and the odd positive integers, respectively. However, neither function is an enumeration of $\mathbb{Z}^{+}$, since neither is <bb>surjective</bb>.</div>\EndKnitrBlock{example}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0124"><strong>(\#cnj:prob0124) </strong></span>Define an enumeration of the positive squares $1$, $4$, $9$, $16, \cdots$</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{example}<div class="example"><span class="example" id="exm:exmEnumeration03"><strong>(\#exm:exmEnumeration03) </strong></span>The function $f(n) = (-1)^{n} \lceil \frac{(n-1)}{2}\rceil$ (where $\lceil x \rceil$ denotes the <bb>ceiling</bb> function, which rounds $x$ up to the nearest integer) enumerates the set of integers $\mathbb{Z}$. Notice how $f$ generates the values of $\mathbb{Z}$ by "hopping" back and forth between positive and negative integers:
...
\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0125"><strong>(\#cnj:prob0125) </strong></span>Show that if $A$ and $B$ are <bb>enumerable</bb>, so is $A \cup B$. To do this, suppose there are <bb>surjective</bb> functions $f\colon \mathbb{Z}^{+} \to A$ and $g\colon \mathbb{Z}^{+} \to B$, and define a <bb>surjective</bb> function $h\colon \mathbb{Z}^{+} \to A \cup B$ and prove that it is <bb>surjective</bb>. Also consider the cases where $A$ or $B = \varnothing$.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0126"><strong>(\#cnj:prob0126) </strong></span>Show that if $B \subseteq A$ and $A$ is <bb>enumerable</bb>, so is $B$. To do this, suppose there is a <bb>surjective</bb> function $f\colon \mathbb{Z}^{+} \to A$. Define a <bb>surjective</bb> function $g\colon \mathbb{Z}^{+} \to B$ and prove that it is <bb>surjective</bb>. What happens if $B = \varnothing$?</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0127"><strong>(\#cnj:prob0127) </strong></span>Show by induction on $n$ that if $A_1$, $A_2, \cdots, A_n$ are all <bb>enumerable</bb>, so is $A_1 \cup \cdots \cup A_n$. You may assume the fact that if two sets $A$ and $B$ are <bb>enumerable</bb>, so is $A \cup B$.</div>\EndKnitrBlock{conjecture}
...
\BeginKnitrBlock{proposition}<div class="proposition"><span class="proposition" id="prp:prpsurjection"><strong>(\#prp:prpsurjection) </strong></span>There is a <bb>surjection</bb> $f\colon \mathbb{Z}^{+} \to A$ iff there is a <bb>surjection</bb> $g\colon \mathbb{N} \to A$.</div>\EndKnitrBlock{proposition}

\BeginKnitrBlock{proof}<div class="proof">\iffalse{} <span class="proof"><em>Proof: </em></span>  \fi{}Given a <bb>surjection</bb> $f\colon \mathbb{Z}^{+} \to A$, we can define $g(n) = f(n+1)$ for all $n \in \mathbb{N}$. It is easy to see that $g\colon \mathbb{N} \to A$ is <bb>surjective</bb>. Conversely, given a <bb>surjection</bb> $g\colon \mathbb{N} \to A$, define $f(n) = g(n+1)$.</div>\EndKnitrBlock{proof}

This gives us the following result:

\BeginKnitrBlock{corollary}<div class="corollary"><span class="corollary" id="cor:corsurjection"><strong>(\#cor:corsurjection) </strong></span>A set $A$ is <bb>enumerable</bb> iff it is empty or there is a <bb>surjective</bb> function $f\colon \mathbb{N} \to A$.</div>\EndKnitrBlock{corollary}
...
\BeginKnitrBlock{corollary}<div class="corollary"><span class="corollary" id="cor:corenumerable"><strong>(\#cor:corenumerable) </strong></span>A set $A$ is <bb>enumerable</bb> iff it is empty or there is a <bb>bijection</bb> $f\colon N \to A$ where either $N = \mathbb{N}$ or $N = \lbrace 0, \cdots, n\rbrace$ for some $n \in \mathbb{N}$.</div>\EndKnitrBlock{corollary}

\BeginKnitrBlock{proof}<div class="proof">\iffalse{} <span class="proof"><em>Proof: </em></span>  \fi{}$A$ is <bb>enumerable</bb> iff $A$ is empty or there is a <bb>surjective</bb> $f\colon \mathbb{Z}^{+} \to A$. By Proposition 1.12, the latter holds iff there is a <bb>bijective</bb> function $f\colon Z \to A$ where $Z = \mathbb{Z}^{+}$ or $Z = \lbrace 1, \cdots, n\rbrace$ for some $n \in \mathbb{Z}^{+}$. By the same argument as in the proof of Proposition 1.11, that in turn is the case iff there is a <bb>bijection</bb> $g\colon N \to A$ where either $N = \mathbb{N}$ or $N = \lbrace 0, \cdots, n-1\rbrace$.</div>\EndKnitrBlock{proof}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0128"><strong>(\#cnj:prob0128) </strong></span>According to Definition1.48（P.63）, a set $A$ is enumerable iff $A = \varnothing$ or there is a <bb>surjective</bb> $f\colon \mathbb{Z}^{+} \to A$.  It is also possible to define "<bb>enumerable</bb> set" precisely by: a set is enumerable iff there is a <bb>injective</bb> function $g\colon A \to \mathbb{Z}^{+}$.  Show that the definitions are equivalent, i.e., show that there is a <bb>injective</bb> function $g\colon A \to \mathbb{Z}^{+}$ iff either $A = \varnothing$ or there is a <bb>surjective</bb> $f\colon \mathbb{Z}^{+} \to A$.</div>\EndKnitrBlock{conjecture}
...
\BeginKnitrBlock{proposition}<div class="proposition"><span class="proposition" id="prp:prpntimesn"><strong>(\#prp:prpntimesn) </strong></span>$\mathbb{N} \times \mathbb{N}$ is <bb>enumerable</bb>.</div>\EndKnitrBlock{proposition}

\BeginKnitrBlock{proof}<div class="proof">\iffalse{} <span class="proof"><em>Proof: </em></span>  \fi{}Let $f \colon \mathbb{N} \to \mathbb{N}\times\mathbb{N}$ take each $k \in \mathbb{N}$ to the tuple $⟨n,m⟩ \in \mathbb{N} \times \mathbb{N}$ such that $k$ is the value of the $n$th row and $m$th column in Cantor's zig-zag array. </div>\EndKnitrBlock{proof}
...
\BeginKnitrBlock{proposition}<div class="proposition"><span class="proposition" id="prp:prpntimesnns"><strong>(\#prp:prpntimesnns) </strong></span>$\mathbb{N}^n$ is <bb>enumerable</bb>, for every $n \in \mathbb{N}$.</div>\EndKnitrBlock{proposition}
...
\BeginKnitrBlock{definition}\iffalse{-91-80-97-105-114-105-110-103-32-102-117-110-99-116-105-111-110-93-}\fi{}<div class="definition"><span class="definition" id="def:defPairingf"><strong>(\#def:defPairingf)  \iffalse (Pairing function) \fi{} </strong></span>A function $f\colon A \times B \to \mathbb{N}$ is an arithmetical <bb>pairing function</bb> if $f$ is injective. We also say that $f$ <bb>encodes</bb> $A \times B$, and that $f(x,y)$ is the <bb>code</bb> for $⟨x,y⟩$.</div>\EndKnitrBlock{definition}
...
\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0129"><strong>(\#cnj:prob0129) </strong></span>Give an enumeration of the set of all non-negative rational numbers. </div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0130"><strong>(\#cnj:prob0130) </strong></span>Show that $\mathbb{Q}$ is <bb>enumerable</bb>. Recall that any rational number can be written as a fraction $z/m$ with $z \in \mathbb{Z}$, $m \in \mathbb{N}^+$.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0131"><strong>(\#cnj:prob0131) </strong></span>Define an enumeration of $\mathbb{B}^*$.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0132"><strong>(\#cnj:prob0132) </strong></span>Recall from your introductory logic course that each possible truth table expresses a truth function. In other words, the truth functions are all functions from $\mathbb{B}^k \to \mathbb{B}$ for some $k$. Prove that the set of all truth functions is enumerable.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0133"><strong>(\#cnj:prob0133) </strong></span>Show that the set of all finite subsets of an arbitrary infinite <bb>enumerable</bb> set is <bb>enumerable</bb>.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0134"><strong>(\#cnj:prob0134) </strong></span>A subset of $\mathbb{N}$ is said to be <bb>cofinite</bb> iff it is the complement of a finite set $\mathbb{N}$; that is, $A \subseteq \mathbb{N}$ is cofinite iff $\mathbb{N}\setminus A$ is finite. Let $I$ be the set whose <bb>element</bb>s are exactly the finite and cofinite subsets of $\mathbb{N}$. Show that $I$ is <bb>enumerable</bb>.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0135"><strong>(\#cnj:prob0135) </strong></span>Show that the <bb>enumerable</bb> union of <bb>enumerable</bb> sets is <bb>enumerable</bb>. That is, whenever $A_1$, $A_2$, \cdots{} are sets, and each $A_i$ is <bb>enumerable</bb>, then the union $\bigcup_{i=1}^\infty A_i$ of all of them is also <bb>enumerable</bb>. [NB: this is hard!]</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0136"><strong>(\#cnj:prob0136) </strong></span>Let $f \colon A \times B \to \mathbb{N}$ be an arbitrary pairing function. Show that the inverse of $f$ is an enumeration of $A \times B$.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0137"><strong>(\#cnj:prob0137) </strong></span>Specify a function that encodes $\mathbb{N}^3$.</div>\EndKnitrBlock{conjecture}
...
\BeginKnitrBlock{example}<div class="example"><span class="example" id="exm:exmpairingfunction01"><strong>(\#exm:exmpairingfunction01) </strong></span>The function $h\colon \mathbb{N}^2 \to \mathbb{N}$ given by $$h(n,m) = 2^n (2m+1) - 1$$ is a pairing function for the set of pairs of natural numbers $\mathbb{N}^2$.</div>\EndKnitrBlock{example}
...
\BeginKnitrBlock{example}<div class="example"><span class="example" id="exm:exmpairingfunction02"><strong>(\#exm:exmpairingfunction02) </strong></span>The function $j\colon \mathbb{N}^2 \to \mathbb{N}^+$ given by $$j(n,m) = 2^n3^m$$ is a <bb>injective</bb> function $\mathbb{N}^2 \to \mathbb{N}$.</div>\EndKnitrBlock{example}
...
\BeginKnitrBlock{theorem}<div class="theorem"><span class="theorem" id="thm:thmnonenumerable"><strong>(\#thm:thmnonenumerable) </strong></span>$\mathbb{B}^\omega$ is <bb>nonenumerable</bb>.</div>\EndKnitrBlock{theorem}

\BeginKnitrBlock{proof}<div class="proof">\iffalse{} <span class="proof"><em>Proof: </em></span>  \fi{}Suppose, by way of contradiction, that $\mathbb{B}^\omega$ is <bb>enumerable</bb>, i.e., suppose that there is a list $s_{1}$, $s_{2}$, $s_{3}$, $s_{4}, \cdots$, of all <bb>element</bb>s of $\mathbb{B}^\omega$.  Each of these $s_i$ is itself an infinite sequence of $0$'s and $1$'s. Let's call the $j$-th element of the $i$-th sequence in this list $s_i(j)$. Then the $i$-th sequence $s_i$ is $$s_i(1), s_i(2), s_i(3), \cdots$$
...
\BeginKnitrBlock{theorem}<div class="theorem"><span class="theorem" id="thm:thmenumerable01"><strong>(\#thm:thmenumerable01) </strong></span>$\mathscr{P}({\mathbb{Z})^{+}}$ is not <bb>enumerable</bb>.</div>\EndKnitrBlock{theorem}

\BeginKnitrBlock{proof}<div class="proof">\iffalse{} <span class="proof"><em>Proof: </em></span>  \fi{}We proceed in the same way, by showing that for every list of subsets of $\mathbb{Z}^{+}$ there is a subset of $\mathbb{Z}^{+}$ which cannot be on the list. Suppose the following is a given list of subsets of $\mathbb{Z}^{+}$: $$ Z_{1}, Z_{2}, Z_{3}, \cdots $$ We now define a set $\overline{Z}$ such that for any $n \in \mathbb{Z}^{+}$, $n \in \overline{Z}$ iff $n \notin Z_{n}$: $$ \overline{Z} = \lbrace n \in \mathbb{Z}^{+}: n \notin Z_n\rbrace$$ </div>\EndKnitrBlock{proof}

$\overline{Z}$ is clearly a set of positive integers, since by assumption each $Z_n$ is, and thus $\overline{Z} \in \mathscr{P}({\mathbb{Z})^{+}}$. But $\overline{Z}$ cannot be on the list. To show this, we'll establish that for each $k \in \mathbb{Z}^{+}$, $\overline{Z} \neq Z_k$. 
...
\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0138"><strong>(\#cnj:prob0138) </strong></span>Show that $\mathscr{P}({\mathbb{N})}$ is <bb>nonenumerable</bb> by a diagonal argument.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0139"><strong>(\#cnj:prob0139) </strong></span>Show that the set of functions $f \colon \mathbb{Z}^{+} \to \mathbb{Z}^{+}$ is <bb>nonenumerable</bb> by an explicit diagonal argument. That is, show that if $f_1$, $f_2$, \cdots, is a list of functions and each $f_i\colon \mathbb{Z}^{+} \to \mathbb{Z}^{+}$, then there is some $\overline{f}\colon \mathbb{Z}^{+} \to \mathbb{Z}^{+}$ not on this list.</div>\EndKnitrBlock{conjecture}
...
\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0140"><strong>(\#cnj:prob0140) </strong></span>Show that if there is an <bb>injective</bb> function $g\colon B \to A$, and $B$ is <bb>nonenumerable</bb>, then so is $A$. Do this by showing how you can use $g$ to turn an enumeration of $A$ into one of $B$.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{proof}\iffalse{-91-80-114-111-111-102-32-111-102-32-84-104-101-111-114-101-109-32-50-46-51-32-98-121-32-114-101-100-117-99-116-105-111-110-93-}\fi{}<div class="proof">\iffalse{} <span class="proof"><em>Proof</em> (Proof of Theorem 2.3 by reduction): </span>  \fi{}Suppose that $\mathscr{P}({\mathbb{Z})^{+}}$ were <bb>enumerable</bb>, and thus that there is an enumeration of it, $Z_{1}$, $Z_{2}$, $Z_{3}, \cdots$
...
So if $\mathscr{P}({\mathbb{Z})^{+}}$ were <bb>enumerable</bb>, $\mathbb{B}^\omega$ would be <bb>enumerable</bb>.  But $\mathbb{B}^\omega$ is <bb>nonenumerable</bb> (Theorem \@ref(thm:thmnonenumerable)). Hence $\mathscr{P}({\mathbb{Z})^{+}}$ is <bb>nonenumerable</bb>. $\blacksquare$</div>\EndKnitrBlock{proof}
...
\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0141"><strong>(\#cnj:prob0141) </strong></span>Show that the set of all <bb>sets of</bb> pairs of positive integers is <bb>nonenumerable</bb> by a reduction argument.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0142"><strong>(\#cnj:prob0142) </strong></span>Show that $\mathbb{N}^\omega$, the set of infinite sequences of natural numbers, is <bb>nonenumerable</bb> by a reduction argument.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0143"><strong>(\#cnj:prob0143) </strong></span>Let $P$ be the set of functions from the set of positive integers to the set $\lbrace 0\rbrace$, and let $Q$ be the set of <bb>partial</bb> functions from the set of positive integers to the set $\lbrace 0\rbrace$. Show that $P$ is <bb>enumerable</bb> and $Q$ is not. (Hint: reduce the problem of enumerating $\mathbb{B}^\omega$ to enumerating $Q$).</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0144"><strong>(\#cnj:prob0144) </strong></span>Let $S$ be the set of all <bb>surjective</bb> functions from the set of positive integers to the set $\lbrace 0,1\rbrace$, i.e., $S$ consists of all <bb>surjective</bb> $f\colon \mathbb{Z}^{+} \to \mathbb{B}$.  Show that $S$ is <bb>nonenumerable</bb>.</div>\EndKnitrBlock{conjecture}

\BeginKnitrBlock{conjecture}<div class="conjecture"><span class="conjecture" id="cnj:prob0145"><strong>(\#cnj:prob0145) </strong></span>Show that the set $\mathbb{R}$ of all real numbers is <bb>nonenumerable</bb>.</div>\EndKnitrBlock{conjecture}
